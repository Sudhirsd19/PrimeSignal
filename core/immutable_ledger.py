...
class ImmutableLedger:
    ...
    Hash-chain mutation is serialized so concurrent position/exit tasks cannot
    compute the same ``prev_hash`` and corrupt the ledger chain.
    """
                ...
                with open(self.ledger_file, 'r', encoding='utf-8') as f:
                    for line in f:
                        ...
                        if event_type == 'POSITION_OPENED':
                            ...
                            raw_payload = (
                                f"{current_hash}|{record.get('record_id')}|{record.get('symbol')}|"
                                f"{record.get('exit_qty')}|{record.get('exit_price')}|{record.get('realized_pnl')}|{record.get('exit_reason')}|{record.get('timestamp')}"
                            )
                        else:
                            raise ValueError(
                                f"CRITICAL: Immutable ledger integrity check failed! "
